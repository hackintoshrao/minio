...
func testAPIGetOjectHandler(obj ObjectLayer, instanceType, bucketName string, apiRouter http.Handler,
	...
	for i, testCase := range testCases {
		...
		if rec.Code != testCase.expectedRespStatus {
			t.Fatalf("Case %d: Minio %s: Expected the response status to be `%d`, but instead found `%d`", i+1, instanceType, testCase.expectedRespStatus, rec.Code)
		}
		...
		// Verify whether the obtained object is same as the one inserted.
...
func testAPINewMultipartHandler(obj ObjectLayer, instanceType, bucketName string, apiRouter http.Handler,
	...
	if err != nil {
		t.Fatal("Invalid UploadID: <ERROR> %s", err)
	}
...
func testAPICompleteMultipartHandler(obj ObjectLayer, instanceType, bucketName string, apiRouter http.Handler,
	...
	testCases := []struct {
		...
		// 404 Not Found response status expected.
		{
			...
			uploadID: "abc",
			parts:    inputParts[0].parts,
			expectedContent: encodeResponse(invalidUploadIDAPIError{"abc",
				getAPIErrorResponse(getAPIError(toAPIErrorCode(InvalidUploadID{UploadID: "abc"})),
					getGetObjectURL("", bucketName, objectName))}),
			expectedRespStatus: http.StatusNotFound,
		},
		// Test case - 4.
		...
		// TestCase with invalid Part Number.
		{
			...
			expectedContent: encodeResponse(invalidPartNumberAPIError{"10",
				getAPIErrorResponse(getAPIError(toAPIErrorCode(InvalidPart{PartNumber: "10"})),
					getGetObjectURL("", bucketName, objectName))}),
			expectedRespStatus: http.StatusBadRequest,
		},
		// Test case - 6.
